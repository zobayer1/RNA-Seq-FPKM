...
- [x] From DE results, define:
  - [x] List of significantly up-regulated genes (e.g., `padj < threshold` and log2FC > cutoff).
  - [x] List of significantly down-regulated genes (e.g., `padj < threshold` and log2FC < -cutoff).
- [x] Optionally create a ranked gene list:
  - [x] Rank genes by log2FC or a signed statistic (e.g., sign(log2FC) * -log10(pvalue)).

### 5.3 Map Gene Identifiers
- [x] Determine which ID type the enrichment tools expect:
  - [x] DAVID, KEGG, GO tools, or R packages like `clusterProfiler` may require Entrez IDs or gene symbols.
- [x] Use annotation resources:
  - [x] Use `AnnotationDbi` and `org.Hs.eg.db` (if human) to map from gene IDs in the dataset to required IDs.
  - [x] Track how many genes successfully map and how many are lost.

### 5.4 Choose Enrichment Tools and Strategy
- [x] Option A: Web-based tools (e.g., DAVID, Enrichr, web-based GO/KEGG portals):
...
- [x] Option B: R-based tools (preferred for reproducibility if allowed/expected):
  - [x] Use `clusterProfiler` for functions like `enrichGO` (GO BP) on up- and down-regulated gene lists.
  - [x] Provide gene lists and universe/background as all expressed genes.
  - [ ] Optionally use MSigDB gene sets with `fgsea` or similar packages.

### 5.5 Run Enrichment / Pathway Analysis
- [x] Run ORA for up-regulated genes:
  - [x] Identify significantly enriched GO terms and pathways.
- [x] Run ORA for down-regulated genes:
  - [x] Identify enriched terms/pathways potentially associated with suppressed functions in tumors.
- [ ] If performing GSEA:
  - [x] Use ranked gene list and chosen gene sets (e.g., Hallmark, KEGG).
  - [ ] Extract top positively and negatively enriched pathways.

### 5.6 Summarize and Visualize Enrichment Results
- [x] Visualization:
  - [x] Create bar plots of top enriched GO terms for up- and down-regulated genes.
  - [ ] Consider enrichment maps or network plots (optional, depending on assignment scope).
- [ ] Interpretation:
...
- [x] Documentation:
  - [x] Record thresholds, databases, and tools used for enrichment.
  - [x] Note limitations (e.g., incomplete mapping, redundancy of GO terms, FPKM-based DE).
